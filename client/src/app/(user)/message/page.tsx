...
const MessagePage: React.FC = () => {
  ...
  const [messageParagraphInfo, setMessageParagraphInfo] =
    React.useState<MessageParagraphInfo>({
      room_id: '',
      messageParagraph: []
    })
  ...
  const handleClickUserListItem = (
    e: React.MouseEvent<HTMLDivElement, MouseEvent>,
    receiverTarget: ReceiverInfo
  ) => {
  ...
  const sendMessage = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    if (messageParagraphInfo.messageParagraph?.length > 0) {
      socket.emit('create', {
        room_id: messageParagraphInfo.room_id,
        content: messageText
      })
    } else {
      messageService
                ...
                renderItem={(user) => (
                  <List.Item
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      backgroundColor: `${
                        user.id === receiver.id ? '#9ea5b5' : '#FFF'
                      }`
                    }}
                    onClick={(e) =>
                      handleClickUserListItem(e, {
                        id: user.id,
